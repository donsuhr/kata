...
const renderList = async () => {
  ...
  const fragments = items.map(item => {
    ...
    const CTA = inCart ? '<div class="menu-list-item__in-cart"><img src="./images/check.svg" alt=""/>In Cart</div>' : `<button class="menu-list-item__add-to-cart" type="button" data-id="${item.id}">
             Add to Cart
          </button>`;
...
const init = async () => {
  ...
  getContainerEl().addEventListener('click', event => {
    ...
    (async () => {
      ...
      if (target?.tagName !== 'BUTTON' || !Object.hasOwn(target?.dataset, 'id')) return;
      const {
        id
      } = target.dataset;
      const items = await getMenuItems();
