...
import { ACTIONS } from '../components/Cart/CartStateProvider.js';
...
describe('cart', () => {
  ...
  it('lets us know it started a save', async () => {
    ...
    expect(mockLocalStorage.setItem).toHaveBeenCalledWith(expect.anything(), expect.stringContaining(serialized));
    expect(dispatch).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('restore', () => {
    it('logs a warn when a stored item is not available', () => {
      ...
      /* eslint-disable no-console */
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('with id 3 not found'));
    ...
    it('logs an error if localstorage throws', () => {
      ...
      /* eslint-disable no-console */
      expect(console.error).toHaveBeenCalledWith(expect.stringMatching(/localstorage error/i), expect.anything());
      console.error.mockRestore();
      /* eslint-enable no-console */
    });
