...
describe('CartStateProvider', () => {
  it('throws when not inside provider', () => {
    ...
    expect(() => renderHook(() => useCartState())).toThrow(
      /within a Provider/i,
    );
  ...
  it("doesn't throw when it has a provider wrapper", () => {
    ...
    expect(() => renderHook(() => useCartState(), { wrapper })).not.toThrow();
  });

  it('calls the persistance layer', async () => {
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <MenuStateProvider initialStatus={STATUS.SUCCESS}>
        <CartStateProvider>{children}</CartStateProvider>
      </MenuStateProvider>
    );
    renderHook(() => useCartState(), { wrapper });
  ...
  it('has a handy hasItemById utility function', () => {
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <MenuStateProvider initialStatus={STATUS.SUCCESS}>
        <CartStateProvider initialCartItems={cart}>
          {children}
        </CartStateProvider>
      </MenuStateProvider>
    );
  ...
  describe('Reducer', () => {
    ...
    it('throws an error for an unknown action with incorrect payload / non array payload', () => {
      const action: ItemReducerAction = {
        // ADD is unknown for CartItem[] Array
        type: ACTIONS.ADD,
        // @ts-expect-error: testing errors
        payload: { id: 'bar', title: 'foo' },
      };

      expect(() => itemsReducer(startingState, action)).toThrow(
        /unknown argument/i,
      );
    });
    ...
    describe('ADD', () => {
      ...
      describe('START_SAVE', () => {
        it('sets the status to "loading" on START_SAVE', () => {
          ...
          const startingStateWithItem = {
            ...startingState,
            cart: [{ ...result.cart[0], status: STATUS.DIRTY }],
            totals: {
              subTotal: 1,
              taxTotal: 1 * TAX_RATE,
              grandTotal: 1 * TAX_RATE + 1,
      ...
      describe('SAVE_SUCCESS', () => {
        it('sets the status to "success" on START_SAVE', () => {
          ...
          const startingStateWithItem = {
            ...startingState,
            cart: [{ ...result.cart[0], status: STATUS.LOADING }],
            totals: {
              subTotal: 1,
              taxTotal: 1 * TAX_RATE,
              grandTotal: 1 * TAX_RATE + 1,
        ...
        it('accounts for race condition of update after save', () => {
          ...
          const firstSaveAction: ItemReducerAction = {
            type: ACTIONS.START_SAVE,
            payload: {
              ...typicalItem,
            },
          ...
          const secondEditAction: ItemReducerAction = {
            type: ACTIONS.UPDATE,
            payload: {
              ...typicalItem,
          ...
          const thirdSaveSuccessAction: ItemReducerAction = {
            type: ACTIONS.SAVE_SUCCESS,
            payload: {
              ...typicalItem,
            },
          ...
          // 1, start a save
          const firstState = itemsReducer(
            startingStateWithItem,
            firstSaveAction,
          );
          ...
          expect(itemsReducer(secondState, thirdSaveSuccessAction)).toEqual(
            result,
          );
        });
      });
