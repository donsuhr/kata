...
describe('MenuStateProvider', () => {
  it('throws when not inside provider', () => {
    ...
    expect(() => renderHook(() => useMenuState())).toThrow(
      /within a Provider/i,
    );
  ...
  it("doesn't throw when it has a provider wrapper", () => {
    ...
    expect(() => renderHook(() => useMenuState(), { wrapper })).not.toThrow();
  });

  it('calls though to the getItems and updates items', async () => {
    const foodItem: ProductsService.FoodMenuItem = {
      id: '2',
      title: 'target title',
      price: 2.09,
      imageUrl: 'xxx.jpg',
    ...
    const mockGetItems = ProductsService.getItems as jest.MockedFunction<
      typeof ProductsService.getItems
    >;
